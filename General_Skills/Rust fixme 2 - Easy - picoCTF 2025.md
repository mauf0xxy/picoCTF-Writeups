# Rust fixme 2 - Easy - picoCTF 2025
...
┌──(kali㉿kali)-[/tmp/rust_proj/fixme2/src]
└─$ cargo build 
   ...
   Compiling rust_proj v0.1.0 (/tmp/rust_proj/fixme2)
...
error[E0596]: cannot borrow `*borrowed_string` as mutable, as it is behind a `&` reference
  --> src/main.rs:20:5
   |
20 |     borrowed_string.push_str(&String::from_utf8_lossy(&decrypted_buffe...
   |     ^^^^^^^^^^^^^^^ `borrowed_string` is a `&` reference, so it cannot be borrowed as mutable
   |
help: consider changing this to be a mutable reference
   |
...
└─$ cargo run   
    Finished `dev` profile [unoptimized + debuginfo] target(s) in 0.00s
     Running `/tmp/rust_proj/fixme2/target/debug/rust_proj`
Using memory unsafe languages is a: PARTY FOUL! Here is your flag:
```

---

`picoCTF{4r3_y0u_h4v1n5_fun_[REDACTED]}`